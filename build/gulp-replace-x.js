...
module.exports = function (replaceFrom, replaceTo, userOptions) {
    var options = _.merge(true, {}, defaultOptions, userOptions);
    ...
    return through.obj(function (file, enc, callback) {
        ...
        var fileDirPath = file.path.substring(0, file.path.lastIndexOf(path.sep) + 1);

        var _replaceTo = function (replacement) {
            if (typeof replaceTo === 'function') {
                var replaceFunctionResult = replaceTo.apply({
                    filePath:file.path,
                    fileDirPath,
                    file,
                }, arguments);
        ...
        if (file.isStream()) {
            file.contents = file.contents.pipe(replaceStream(replaceFrom, _replaceTo));
